...
def get_feats_with_proper_labels(path, labelkind):
    ...
    if "type_cls6_str" in gf_feats.columns:
        label_column = "type_cls6_str"
    elif "label_easy5" in gf_feats.columns:
        label_column = "label_easy5"
    elif "type" in gf_feats.columns:
        label_column = "type"
    else:
    ...
    if isinstance(labelkind, str):
        ...
        if labelkind == "easy2":
            gf_feats["label"] = gf_feats[label_column]
...
def gpkg_save(gf, fold, name):
    gf = gf.copy()
    if len(gf):
        gf.index.name = "fid"  # QGIS compatability

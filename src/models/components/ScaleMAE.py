...
class FCNHead(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_layers, target_dim) -> None:
        ...
        self.pred = nn.Sequential(
            Norm2d(hidden_dim),
            nn.ConvTranspose2d(
                hidden_dim, hidden_dim // 2, kernel_size=4, stride=4
            ),
            nn.GELU(),
            nn.Conv2d(
                hidden_dim // 2,
...
class ScaleVit(VisionTransformer):
    ...
    def __init__(self, patch_size=16, in_chans=3, embed_dim=1024, **kwargs):
...
class MaskedScaleViT(nn.Module):
    ...
    def __init__(
        ...
        self.mask_token_decoder = nn.Parameter(
            torch.zeros(1, 1, decoder_embed_dim)
        )
        ...
        if self.multiscale:
            raise ValueError(
                "MultiScale prediction is not supported (need to implement)"
            )
    ...
    def upsample_decoder(self, x, target_dim):
        ...
        x = torch.nn.functional.interpolate(
            input=x.reshape(
                n, 1, l_low_dim, l_low_dim, self.decoder_embed_dim
            ),
    ...
    def plot_decoder_vector(self, x):
        ...
        for img in range(B):
            for i in range(embed_dim):
                output_raster[
                    img,
                    i * num_patches_per_axis : (i + 1) * num_patches_per_axis,
                    :,
                ] = data[img, i, :, :]
    ...
    def forward_decoder(
        ...
        if not self.use_mask_token:  # drop mask token from encoder
            ...
            mask_idx = torch.argsort(mask, dim=-1, descending=True)[
                :, :num_masked
            ]
        ...
        # standard decode
        x = (
            x.view(n, p, p, d).permute(0, 3, 1, 2).contiguous()
        )  # B X C X H X W
    ...
    def split_pred(self, pred):
        ...
        pred_all = (
            nn.functional.interpolate(
                nn.functional.interpolate(
                    pred_low,
                    (self.band_config[0], self.band_config[0]),
                    mode="area",
                ),
                pred_high.shape[-2:],
                mode="bilinear",
        ...
        out = pred_high, pred_low, pred_all
        return out
    ...
    def build_input_sequence(self, x, base_res, base_dim, pos_emb_base):
        ...
        atten_mask = [
            torch.ones(
                (l_x + length_pos_embed, l_x + length_pos_embed),
                device=x.device,
            )
        ]
        ...
        ids = [mask]
        target_sizes = [
            x for x in self.target_size if x != max(self.target_size)
        ]
        for d in target_sizes:
            ...
            atten_mask.append(
                torch.ones(
                    (length_pos_embed, length_pos_embed), device=x.device
                )
            )
    ...
    def forward_loss(self, imgs, pred, mask, target_dim, ids):
        ...
        if self.use_l1_loss:
            loss_l1 = (
                mask_high
                * torch.abs(target_high - pred_high)
                * self.l1_loss_weight
            )
        else:
    ...
    def forward(
        ...
        if targets is None:
            ...
            imgs = nn.functional.interpolate(
                imgs, imgs.shape[2] // 2, mode="area"
            )
            input_res = input_res * 2
        p = self.patch_embed.patch_size[0]
        target_dim = targets.shape[2] // p
        assert (
            target_res is not None
        ), "if target is specify, target_res should be also specify"
        assert (
            targets.shape[2] == imgs.shape[2] * 2
        ), "The target must have twice as many pixels as the input per side"
        ...
        if return_loss:
            loss, mean, var = self.forward_loss(
                targets, pred, mask, target_dim, ids
            )
            pred = self.split_pred(pred)
            return pred, mask, loss
        else:
            pred = self.split_pred(pred)
            return pred, mask, torch.nan


class ScaleVitNet(nn.Module):
    ...
    def forward(self, x, metas=None):
        ...
        """
        area = [meta["square"].area for meta in metas]
        area = torch.tensor(area, device=x.device)
        side = area.sqrt()
        resolution = side / x.shape[-1]

        x = self.model.forward_features(x, input_res=resolution)

        x = self.seg_head(x)
        return {"out": x}


class ScaleMAENet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_size = patch_size
        self.use_scale_loss = use_scale_loss

        self.model = MaskedScaleViT(
            img_size=img_size // 2,
            patch_size=patch_size,
            in_chans=num_channels,
            embed_dim=embed_dim,
    ...
    def forward(self, x, metas=None):
        ...
        preds, mask, loss = self.model.forward(
            x,
            mask_ratio=self.MAE_drop_prob,
            input_res=resolution,
            return_loss=self.use_scale_loss,
        )
        pred_high, pred_low, pred_all = preds
